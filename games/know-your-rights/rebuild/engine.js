...
window.KnowYourRights.init = function (root, base, opts) {
  ...
  function renderCard(){
    ...
    if (card.type==='end'){ showBackdropFor(card); return sayResponses(card, function(){ finish(!!card.fatal); }); }
    showBackdropFor(card);
    S.cardsSeen[S.cardId]=true; S.pathLen++; drawDots();
    sayResponses(card, function(){ showChoices(card); });
  }
  function pickText(resp){ return (resp.mode==='random') ? pick(resp.texts) : resp.texts[0]; }
  // Plays a card's lead response line, then (if present) one
  // narration-on-shield follow-up line, then hands off to `then`.
  // Shared by mid-scene cards and end cards, so end-card flavor text
  // (e.g. door.json's `leave`/`opened-early`) actually renders instead
  // of being skipped straight to the result screen.
  function sayResponses(card, then){
    ...
    say(pickText(resp), resp.speaker==null, function(){
      if (extra.length) return say(pickText(extra[0]), true, then);
      then();
    });
  }
  ...
  function chooseAnswer(a){
    ...
    // behind the `why` line and the end card's own flavor text, then finish detained.
    if (a.grade==='fatal'){ var pk2=primaryKey(); S.meters[pk2]=maxOf(pk2);
      S.cardId = a.goto || S.cardId;
      var endCard=curCard();
      showBackdropFor(endCard);
      var afterWhy=function(){ sayResponses(endCard, function(){ finish(true); }); };
      if (a.why) return say(a.why, true, afterWhy);
      return afterWhy(); }
